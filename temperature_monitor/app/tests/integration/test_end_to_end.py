...
def test_end_to_end_temperature_workflow(client):
    ...
    response = client.post(
        "/auth/register", json={"username": "e2e_admin", "email": "e2e_admin@example.com", "password": "password123"}
    )
    assert response.status_code == 201

    # 2. Login with the new user
    response = client.post("/auth/login", json={"username": "e2e_admin", "password": "password123"})
    ...
    response = client.post(
        "/api/temperature", headers=headers, json={"sensor_id": "e2e-sensor", "temperature": 24.5, "humidity": 65.0}
    )
    assert response.status_code == 201

    # 4. Query temperature data
    response = client.get("/api/temperature/e2e-sensor?timeframe=daily", headers=headers)
    assert response.status_code == 200
    data = json.loads(response.data)
    assert data["sensor_id"] == "e2e-sensor"

    # 5. Update user profile
    response = client.put("/auth/profile", headers=headers, json={"theme": "dark", "dashboard_layout": "compact"})
    assert response.status_code == 200
    data = json.loads(response.data)
    ...
    response = client.post(
        "/admin/settings",
        headers=headers,
        json={"key": "e2e_setting", "value": "test_value", "description": "E2E test setting", "requires_admin": False},
    )
    assert response.status_code == 201

    # 7. Register a regular user
    response = client.post(
        "/auth/register", json={"username": "e2e_user", "email": "e2e_user@example.com", "password": "password123"}
    )
    assert response.status_code == 201

    # 8. Login with regular user
    response = client.post("/auth/login", json={"username": "e2e_user", "password": "password123"})
    assert response.status_code == 200
    data = json.loads(response.data)
    ...
    response = client.get("/api/temperature/e2e-sensor?timeframe=daily", headers=user_headers)
    assert response.status_code == 200
    ...
    response = client.post(
        ...
        json={"sensor_id": "e2e-sensor-2", "temperature": 25.0, "humidity": 60.0},
    )
    assert response.status_code == 403  # Forbidden

    # 11. Regular user can update non-admin settings
    response = client.put("/admin/settings/e2e_setting", headers=user_headers, json={"value": "updated_value"})
    assert response.status_code == 200

    # 12. Admin can update any user's data
    response = client.put(f"/admin/users/2", headers=headers, json={"theme": "light"})
    assert response.status_code == 200
