def test_input_validation_registration(client):
    ...
    response = client.post(
        "/auth/register", json={"username": "test_validation", "email": "invalid-email", "password": "password123"}
    )
...
def test_input_validation_temperature(client, admin_headers):
    ...
    response = client.post(
        "/api/temperature", headers=admin_headers, json={"sensor_id": "test-sensor", "temperature": "not a number"}
    )
